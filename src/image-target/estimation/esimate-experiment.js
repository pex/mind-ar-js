...
import { computeScreenCoordiate } from './utils.js';
...
const estimate = ({ screenCoords, worldCoords, projectionTransform }) => {
  ...
  const npaNorm = Math.sqrt(
    npa[0] * npa[0] + npa[1] * npa[1] + npa[2] * npa[2]
  );
  const npbNorm = Math.sqrt(
    npb[0] * npb[0] + npb[1] * npb[1] + npb[2] * npb[2]
  );
